...
export default function Home(){
 ...
 const filterControls=<div className="filter-content">
  ...
  <div className="field-label spaced">収録状況</div><Choice label="収録状況で絞り込み" value={filters.availability} onChange={v=>setFilters(current=>({...current,availability:v as Filters['availability']}))} items={[{value:'all',label:'すべて（未収録・不明を含む）'},{value:'available',label:'AC収録譜面のみ'},{value:'removed',label:'AC未収録譜面'}]}/>
 ...
 const editor=<div className={`editor ${mode==='DP'?'dp-editor':''}`}>
  ...
  <div className="editor-song"><div className="song-micro">{seriesCode(activeSong.series)} / {seriesNames[activeSong.series]}{selected.importedInfo?.arcadeAvailability?<span className="removed-label">{chartAvailability(selected,activeSong)==='included'?'AC収録':chartAvailability(selected,activeSong)==='not_included'?'AC未収録':'収録状況不明'} · #{selected.importedInfo.arcadeAvailability.referenceVersion}基準</span>:<>{activeSong.availabilityUnknown&&<span className="removed-label">収録状況不明</span>}{activeSong.removed&&<span className="removed-label">{catalog?'AC未収録':'削除曲'}</span>}</>}</div><h2 style={titleStyle(activeSong.series)}>{activeSong.title}</h2><p>{activeSong.artist}</p><div className="editor-meta"><span className="mode-badge">{mode}</span>{diffBadge(selected.difficulty)}<span className="editor-level">☆{selected.level||'?'}</span><span className="bpm">BPM {selected.bpm??activeSong.bpm}</span><ChartFeatures chart={selected}/>{!!selected.manualApplied?.length&&<span className="manual-badge">譜面データ手入力</span>}{favoritePicker(activeSong.id)}</div></div>
 ...
 return <div className="app-shell" data-iidx-app="1">
  ...
  {activityOpen?<MyActivity charts={charts} songs={songs} records={playRecords} initialMode={mode} source={scoreSource} sources={activitySources} onSource={value=>setViewField('scoreSource',value)} ready={ready&&viewReady} sample={!catalog} error={storageError} disabled={busy||editorOpen||settingsOpen||!!pendingAction} onChart={openActivityChart} onBack={()=>{window.history.replaceState(null,'',window.location.pathname+window.location.search);setActivityOpen(false);}}/>:<div className="workspace">
   ...
   <main className="library-main">
    ...
    {filterCount>0&&<div className="active-filters">{filters.series!=='all'&&<span>{seriesNames[Number(filters.series)]}</span>}{filters.levels.length>0&&<span>{filters.levels.map(value=>`☆${value}`).join('・')}</span>}{!!normalized(filters.artist)&&<span>アーティスト：{filters.artist.trim()}</span>}{filters.availability!=='all'&&<span>{filters.availability==='removed'?'AC未収録譜面':'AC収録譜面'}</span>}{filters.soflan&&<span>ソフラン</span>}{filters.features.map(feature=><span key={feature}>{feature}あり</span>)}{playStatus!=='all'&&<span>{playStatus==='scored'?'スコア記録あり':'クリア：'}{playStatus==='scored'?'':playStatus==='played'?'プレー済み':playStatus==='missing'?'未取得':lampLabels[playStatus as keyof typeof lampLabels]}</span>}{rank!=='all'&&<span>非公式：{rankItems.find(r=>r.value===rank)?.label??'未掲載'}</span>}{radarRanges.map((r,i)=>r.min!==''||r.max!==''?<span key={i}>{radarAxes[i].label} {r.min||'0'}〜{r.max||'200'}</span>:null)}<button disabled={!detailFiltersChanged} onClick={()=>updateView(v=>({...v,filters:defaultFilters(),playStatus:'all',rank:'all',radarRanges:v.radarRanges.map(()=>({min:'',max:''}))}))} aria-label="詳細フィルターを初期値に戻す" title="収録状況はAC収録曲のみに戻します"><RotateCcw size={14}/></button></div>}
    ...
    <div className={`chart-list ${radarActive?'is-radar':''}`} aria-label="楽曲・譜面一覧"><div className="list-head"><span>楽曲 / アーティスト</span><span>難度 / BPM</span><span>保存済みオプション</span><span/></div>
     {results.slice(0,visibleCount).map(c=>{const s=songById[c.songId],note=app.notes[noteKey(c,spSide)],resolvedNote=effectiveNumberNote(note,chartCommonNumbers(app,c,s,spSide),c.mode,spSide);return <article key={c.id} className={`chart-row ${selected.id===c.id?'selected':''}`}>
      <button className="chart-open" aria-label={`${s.title} ${c.mode} ${c.difficulty} レベル${c.level||'?'}の設定を開く`} onClick={()=>selectChart(c)} disabled={!ready||busy}>
       ...
       <span className="row-details"><span className="track-tags"><span className="series-badge">{seriesCode(s.series)}<span>{seriesNames[s.series]}</span></span>{(c.soflan??s.soflan)&&<span className="soflan-tag" role="img" aria-label="ソフラン曲" title="ソフラン曲"><Activity size={14} aria-hidden="true"/></span>}{(c.retained||s.retained)&&<span className="retained-label">前回DB</span>}{c.importedInfo?.arcadeAvailability?<span className="removed-label chart-availability" title={`IIDX ${c.importedInfo.arcadeAvailability.referenceVersion}基準の譜面収録状況`}>{chartAvailability(c,s)==='included'?'AC収録':chartAvailability(c,s)==='not_included'?'未収録':'不明'} #{c.importedInfo.arcadeAvailability.referenceVersion}</span>:<>{s.availabilityUnknown&&<span className="removed-label">収録状況不明</span>}{s.removed&&<span className="removed-label">{catalog?'AC未収録':'削除曲'}</span>}</>}{showUnofficial&&<CommunityRating chart={c} table={table} gauge={gauge} cpiGauge={cpiGauge}/>}</span>{!radarActive&&<span className={`chart-saved ${note?'has-note':''}`}><OptionSummary showLift={app.liftVisibility?.[c.mode]??true} note={resolvedNote} mode={c.mode} side={spSide}/></span>}</span>
